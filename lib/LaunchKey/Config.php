 ...
 * @property  boolean  $use_system_ssl_cert_chain  `TRUE` to use whatever CAs OpenSSL has installed on your system,
 * `FALSE` to use the `ca-bundle.crt` file included in LaunchKey itself (recommended and default).
 *
...
class Config extends \ArrayObject
{
    ...
    private $localApiPublicKey = null;

    private $localKeyPair = null;

    private $localRawKeyPair;
    ...
    private function assert_valid_option($option)
    {
        if (!in_array($option, self::$options)) {
            throw new Exception\LaunchKeyException(
                'specified option :option is not recognized',
                array(
                    ':option' => $option,
                )
            );
    ...
    {
        if ($value === null) {
            return $this->localApiPublicKey;
        }

        return $this->localApiPublicKey = new RSAKey($value);
    }
    ...
    public function keypair($value = null)
    {
        if ($value === null) {
            if ($this->localKeyPair === null) {
                $this->localKeyPair = new RSAKey(
                    $this->localRawKeyPair,
                    array('passphrase' => $this['passphrase'])
                );
            }
        ...
        return $this->localRawKeyPair = $value;
    }
